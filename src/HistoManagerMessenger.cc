...
:manager(man)
{
	treeDirectory = new G4UIdirectory("/shieldingsim/tree/", this);
 	treeDirectory->SetGuidance("Commands for setting the tree output.");

	outputPathCmd = new G4UIcmdWithAString("/shieldingsim/tree/setOutputFilePath", this);
	outputPathCmd->SetGuidance("Set path for output ROOT file.");
	outputPathCmd->SetDefaultValue("");

	outputNameCmd = new G4UIcmdWithAString("/shieldingsim/tree/setOutputFileName", this);
